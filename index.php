...
if (!empty($_GET['a'])) {
    ...
    $article_name = $_GET['a'];
    if (strpos(realpath($published_path.$article_name),$published_path)===0) {
        // create an article list with just the requested article
        $article_list = array($published_path.$article_name);
  	    // pull page title from H1 tag in article
        $article_DOM = new DOMDocument();
        $article_DOM->loadHTMLFile($published_path.$article_name);
        $h1 = $article_DOM->getElementsByTagName('h1');
        if ($h1->length>0) {
            $page_title = $h1->item(0)->nodeValue;
        }
    // same test but with the staging path.
    } else if (strpos(realpath($staging_path.$article_name),$staging_path)===0) {
        // create an article list with just the requested article
        $article_list = array($staging_path.$article_name);
        // pull page title from H1 tag in article
        $article_DOM = new DOMDocument();
        $article_DOM->loadHTMLFile($staging_path.$article_name);
        $h1 = $article_DOM->getElementsByTagName('h1');
        if ($h1->length>0) {
            $page_title = $h1->item(0)->nodeValue;
        }
...
<?php
//begin parsing page_links array from config.php
